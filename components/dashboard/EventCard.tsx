...
const EventCard: React.FC<EventCardProps> = ({ event, onClick }) => {
  ...
  return (
    <div
      className="bg-card rounded-[2rem] overflow-hidden border border-border hover:shadow-2xl hover:shadow-primary/5 transition-all group cursor-pointer flex flex-col h-full relative"
      onClick={onClick}
    >
      <div className="relative aspect-[4/3] overflow-hidden">
        ...
        <div className="absolute top-4 right-4 flex items-center gap-1 bg-white/90 backdrop-blur-sm px-2 py-1 rounded-full shadow-sm">
          ...
          <span className="text-[10px] font-bold text-foreground">{event.guestCount}</span>
        </div>
      </div>

      <div className="p-5 flex flex-col flex-1">
        <h4 className="font-black text-foreground leading-tight mb-2 line-clamp-2 min-h-[2.5rem]">{event.eventName}</h4>
        ...
        <div className="mt-auto space-y-2">
          ...
          <div className="flex items-center justify-between pt-3 border-t border-slate-50">
            ...
            <p className="text-sm font-black text-accent">{formatBudget(event.budget)}</p>
          </div>
        </div>
      </div>
    </div>
