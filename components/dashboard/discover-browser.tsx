...
export type DiscoverMode = 'both' | 'vendors' | 'experiences';

/**
 * Vendor/experience browser behind Discover. `mode` pins it to a single
 * catalogue so /vendors and /experiences can reuse it without the top tabs.
 */
export default function DiscoverBrowser({ mode = 'both' }: { mode?: DiscoverMode } = {}) {
  ...
  // Filter States
  const [selectedTopTab, setSelectedTopTab] = useState<'vendors' | 'experiences'>('vendors');
  // When pinned to one catalogue the tabs are hidden, so the mode wins.
  const topTab = mode === 'both' ? selectedTopTab : mode;
  const setTopTab = setSelectedTopTab;
  ...
  // Sized to h-10 so it lines up with the filter dropdowns it shares a row with.
  const searchInput = (
    <div className="relative w-full group">
      ...
      <input
        ...
        className="w-full h-10 bg-card border border-border rounded-xl pl-10 pr-4 text-[13px] font-medium shadow-sm outline-none focus:border-primary/30 transition-all"
      />
    </div>
  );
  ...
  if (isLoading) {
    return (
      <div className="w-full max-w-6xl mx-auto min-w-0 space-y-6 pb-10">
      ...
      {topTab === 'vendors' ? (
        <div className="space-y-6">
          <div className="flex flex-col gap-3 md:flex-row md:items-center">
            <div className="w-full md:w-64 lg:w-72 md:shrink-0">{searchInput}</div>
        ...
        <div className="space-y-6">
          {/* Experiences have no filters, so the search box holds the row alone. */}
          <div className="w-full md:w-64 lg:w-72">{searchInput}</div>
