...
const Inspiration: React.FC = () => {
  ...
  return (
    <div className="max-w-7xl mx-auto space-y-8">
      <div>
        <h2 className="text-3xl font-black tracking-tight text-foreground">Community Inspiration</h2>
      ...
      <div className="columns-1 md:columns-2 lg:columns-3 gap-6 space-y-6">
        {publicEvents.map(item => (
          <Link href={`/dashboard/hosts/inspiration/${item.id}`} key={item.id} className="block relative rounded-[2rem] overflow-hidden group break-inside-avoid shadow-sm hover:shadow-2xl transition-all border border-border bg-card">
