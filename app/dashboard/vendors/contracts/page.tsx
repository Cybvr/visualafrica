    ...
    <div className="max-w-6xl mx-auto space-y-8 pb-20">
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-6">
        <div>
          ...
          <p className="text-slate-500 mt-1">Manage your agreements and client relationships.</p>
        </div>
      ...
      <div className="space-y-4">
        {contracts.length > 0 ? (
          contracts.map((event, index) => {
            const { status, style } = getContractStatus(event);
            const vendorBooking = event.bookedVendors.find(
              bv => bv.vendorId === VENDOR_DASHBOARD_DATA.currentVendorId
            );
            ...
            return (
              <Link
                href={`/dashboard/vendors/contracts/${event.id}`}
                key={event.id}
              ...
              >
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    <h3 className="text-lg font-bold text-slate-900 group-hover:text-orange-600 transition-colors">{event.eventName}</h3>
                    <div className="flex items-center gap-3 text-sm text-slate-500 mt-1">
                      <span className="flex items-center gap-1"><FileText size={14} /> Contract #C{1000 + index}</span>
                      <span>•</span>
                ...
                <div className="flex flex-wrap items-center gap-6 md:gap-12 w-full md:w-auto">
                  ...
                  <div>
                    ...
                    <div className="font-bold text-slate-900">{vendorBooking?.amount || "Custom Quote"}</div>
                  </div>
          ...
          <div className="text-center py-16 bg-white rounded-[2rem] border border-slate-100">
            ...
            <h3 className="text-lg font-bold text-slate-900 mb-2">No Contracts Yet</h3>
            <p className="text-slate-500">Your confirmed contracts will appear here.</p>
          </div>
        )}
      </div>
    </div>
