...
const VendorInbox: React.FC = () => {
  ...
  const updateStatus = (chatId: string, newStatus: string) => {
    ...
    if (activeChat?.id === chatId) {
      setActiveChat(prev => prev ? { ...prev, status: newStatus } : null);
    }
  };
      ...
      <div className="w-full md:w-80 border-r border-slate-100 flex flex-col">
        <div className="p-6 border-b border-slate-50">
          ...
          <div className="relative">
            ...
            <input type="text" placeholder="Search chats..." className="w-full pl-10 pr-4 py-2 bg-slate-50 border-none rounded-xl text-sm outline-none focus:ring-2 focus:ring-orange-600/20" />
          </div>
        </div>
        <div className="flex-1 overflow-y-auto divide-y divide-slate-50">
          {chats.map((chat) => (
            <button key={chat.id} onClick={() => setActiveChat(chat)} className={`w-full p-6 text-left hover:bg-slate-50 transition-colors flex items-start gap-4 ${activeChat?.id === chat.id ? 'bg-orange-50/50' : ''}`}>
              ...
              <div className="flex-1 min-w-0">
                <div className="flex justify-between items-start mb-1">
                  <div>
                    ...
                    <p className="text-[10px] text-slate-500 font-medium truncate">{chat.eventName}</p>
                  </div>
                ...
                <div className="mt-2 flex items-center justify-between">
                  ...
                  <span className="text-[10px] font-black text-orange-600">{chat.price}</span>
                </div>
                {chat.unread && <div className="mt-2 w-2 h-2 bg-orange-600 rounded-full" />}
              </div>
            </button>
          ))}
        </div>
          ...
          <>
              ...
              <div className="flex items-center gap-2">
                <DropdownMenu>
                  ...
                  <DropdownMenuContent align="end">
                    <DropdownMenuItem onClick={() => updateStatus(activeChat.id, 'pending')}>
                      Mark as Pending
                    </DropdownMenuItem>
                    <DropdownMenuItem onClick={() => updateStatus(activeChat.id, 'confirmed')}>
                      Mark as Confirmed
                    </DropdownMenuItem>
                    <DropdownMenuItem onClick={() => updateStatus(activeChat.id, 'in-progress')}>
                      Mark as In Progress
                    </DropdownMenuItem>
                    <DropdownMenuItem onClick={() => updateStatus(activeChat.id, 'completed')}>
                      Mark as Completed
                    </DropdownMenuItem>
                    <DropdownMenuItem onClick={() => updateStatus(activeChat.id, 'paid')}>
                      Mark as Paid
                    </DropdownMenuItem>
                    <DropdownMenuItem
                ...
                <button className="text-orange-600 text-sm font-black px-4 py-2 hover:bg-orange-50 rounded-xl transition-colors">Profile</button>
              </div>
            </div>
            <div className="flex-1 overflow-y-auto p-8 space-y-6">
              ...
              <div className="flex items-start gap-3 justify-end">
                <div className="bg-orange-600 p-4 rounded-2xl rounded-tr-none shadow-lg shadow-orange-600/10 text-sm text-white leading-relaxed max-w-lg font-bold">
            ...
            <div className="p-6 bg-white border-t border-slate-100">
              <div className="flex gap-4">
                <input type="text" placeholder="Type your response..." className="flex-1 px-6 py-4 bg-slate-50 border-none rounded-2xl outline-none focus:ring-2 focus:ring-orange-600/20 transition-all font-medium" />
                <button className="w-14 h-14 bg-orange-600 text-white rounded-2xl flex items-center justify-center shadow-lg shadow-orange-600/20 hover:scale-105 transition-transform">
                  <Send size={20} />
                </button>
              </div>
            </div>
