    ...
    return (
        <div className="max-w-6xl mx-auto space-y-8">
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                <div>
                    <h2 className="text-3xl font-black tracking-tight text-slate-900">Offers</h2>
                    <p className="text-slate-500 mt-1">Manage your incoming inquiries and potential clients.</p>
                </div>
                <div className="flex items-center gap-2">
                    <div className="relative">
                        ...
                        <input
                            ...
                            className="pl-10 pr-4 py-2 bg-white border border-slate-200 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-primary/20 w-full md:w-64"
                        />
                    </div>
            ...
            <div className="bg-white rounded-[2.5rem] border border-slate-100 shadow-sm overflow-hidden">
                    ...
                    <table className="w-full text-left">
                        <thead>
                            <tr className="border-b border-slate-50">
                                ...
                                <th className="px-6 py-5 text-[10px] font-black text-slate-400 uppercase tracking-widest">Inquiry Type</th>
                                ...
                                <tr key={offer.id} className="hover:bg-slate-50/50 transition-colors group">
                                    <td className="px-8 py-5">
                                        <div className="flex items-center gap-3">
                                            <div className="w-10 h-10 bg-orange-100 rounded-full flex-shrink-0 flex items-center justify-center text-orange-600 font-black text-xs">
                                                {offer.name.charAt(0)}
                                            </div>
                                            <div>
                                                <div className="font-bold text-slate-900">{offer.name}</div>
                                                <div className="text-xs text-slate-500 truncate max-w-[200px]">{offer.detail}</div>
                                            </div>
                                        </div>
                                    </td>
                                    <td className="px-6 py-5">
                                        <span className="text-sm font-medium text-slate-700">{offer.event}</span>
                                    </td>
                                    <td className="px-6 py-5">
                                        <span className="text-sm text-slate-600">{offer.date}</span>
                                    </td>
                                    <td className="px-6 py-5">
                                        <span className={`px-2.5 py-1 rounded-full text-[10px] font-black uppercase tracking-wider ${offer.status === 'New' ? 'bg-blue-100 text-blue-700' :
                                            offer.status === 'Contacted' ? 'bg-orange-100 text-orange-700' :
                ...
                {displayOffers.length === 0 && (
                    <div className="py-20 text-center space-y-4">
                        ...
                        <p className="text-slate-400 font-bold">No {activeTab} offers found.</p>
                    </div>
                )}
            </div>
        </div>
